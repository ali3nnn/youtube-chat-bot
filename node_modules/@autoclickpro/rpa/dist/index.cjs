...
async function getBoxOfScreen(pane) {
  const screenW = await nutJs.screen.width();
  const screenH = await nutJs.screen.height();
...
function box2Region(box) {
  return new nutJs.Region(box.x, box.y, box.width, box.height);
}
async function moveToCenterOfBox(box) {
  await nutJs.mouse.move(nutJs.straightTo(nutJs.centerOf(box2Region(box))));
}
...
async function isAtEndOfPage(grabBox) {
  const first = await nutJs.screen.captureRegion("sc-1.png", box2Region(grabBox));
  await nutJs.keyboard.pressKey(nutJs.Key.PageDown);
  const second = await nutJs.screen.captureRegion("sc-2.png", box2Region(grabBox));
  const {
    equal
  } = await looksSame(first, second);
  if (!equal) {
    await nutJs.keyboard.pressKey(nutJs.Key.PageUp);
    await nutJs.keyboard.releaseKey(nutJs.Key.PageUp);
  }
  return equal;
}
...
async function query(q, option = defaultQueryOption, info = "") {
  ...
  while (Date.now() - now < timeout + (selectTheOneCloseTo ? 6000 : 0)) {
    try {
      ...
      const sc = await nutJs.screen.captureRegion("sc.png", box2Region(grabBox));
      ...
      if (scrollingSearch && !isAtEnd) {
        ...
        await nutJs.keyboard.pressKey(nutJs.Key.PageDown);
        await nutJs.keyboard.releaseKey(nutJs.Key.PageDown);
        await nutJs.mouse.scrollUp(20);
      }
    }
  }
...
function createRpa({
  ...
  nutJs.screen.config.highlightDurationMs = 1000 * 30;
  ...
  return {
    ...
    async clickBox(box, option = {
      ...
      nutJs.mouse.move(nutJs.straightTo(clickPoint));
      await nutJs.mouse.leftClick();
    },
    async click(point) {
      nutJs.mouse.move(nutJs.straightTo(point));
      await nutJs.mouse.leftClick();
    },
    ...
    async clear() {
      await nutJs.keyboard.pressKey(nutJs.Key.LeftCmd, nutJs.Key.A);
      await nutJs.keyboard.releaseKey(nutJs.Key.LeftCmd, nutJs.Key.A);
      await nutJs.keyboard.pressKey(nutJs.Key.Delete);
    },
    async pressKey(...keys) {
      await nutJs.keyboard.pressKey(...keys);
      await nutJs.keyboard.releaseKey(...keys);
    },
    async ocr({
      inPane
    }) {
      const grabBox = await getBoxFromInPane(inPane);
      const sc = await nutJs.screen.captureRegion("dbg/sc.png", box2Region(grabBox));
      const buffer = fs.readFileSync(sc);
      const {
        list
    ...
    async scroll(step) {
      switch (step) {
        ...
        case "pageUp":
          ...
          break;
        case "toBottom":
          await nutJs.keyboard.pressKey(nutJs.Key.End);
          await nutJs.keyboard.releaseKey(nutJs.Key.End);
          break;
        case "toTop":
          await nutJs.keyboard.pressKey(nutJs.Key.Home);
          await nutJs.keyboard.releaseKey(nutJs.Key.Home);
          break;
        default:
          if ("x" in step && "y" in step) {
            ...
            if (y < 0) {
              await nutJs.mouse.scrollUp(-y);
            } else if (y > 0) {
              await nutJs.mouse.scrollDown(y);
            }
            if (x < 0) {
              await nutJs.mouse.scrollLeft(-x);
            } else if (x > 0) {
              await nutJs.mouse.scrollRight(x);
            }
            return;
          }
